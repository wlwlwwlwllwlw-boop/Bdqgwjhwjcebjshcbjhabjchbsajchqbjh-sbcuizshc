-- ============================================
-- WANZZ PROJECT - POTATO ONLY (FIXED)
-- MANUAL BIKE - NO SPAWN BIKE
...
if RemoteEvents then
    ...
    if not Remote then Remote = RemoteEvents:FindFirstChild("RPC") end
    if not Remote then Remote = RemoteEvents:FindFirstChild("UnreliableRemoteEvent") end
end
...
local function VehicleTeleport(targetPosition)
    ...
    if not seat then 
        Configuration.State["Status"] = "[TP] Not on vehicle! Naik bike dulu."
        return false 
    end
...
-- ============================================
-- BELI BAHAN (FIXED - DENGAN VERIFIKASI & RETRY)
-- ============================================

local function PurchasePotatoIngredients()
    WaitForReady()
    
    local StoreRemote = RemoteEvents and RemoteEvents:FindFirstChild("StorePurchase")
    if not StoreRemote then 
        Configuration.State["Status"] = "[ERROR] StorePurchase remote not found!"
        return false 
    end
    ...
    if Potato >= 1 and Flour >= 1 then
        Configuration.State["Status"] = "[POTATO] Already have ingredients."
        return true
    end
    
    Configuration.State["Status"] = "[POTATO] Buying ingredients..."
    
    -- RETRY SAMPAI DAPAT (MAX 5 KALI)
    for attempt = 1, 5 do
        WaitForReady()
        ...
        -- CEK LAGI INVENTORY
        Potato, Flour = ScavengeInventory()
        if Potato >= 1 and Flour >= 1 then
            Configuration.State["Status"] = "[POTATO] Ingredients OK!"
            return true
        end
        ...
        -- VERIFIKASI
        Potato, Flour = ScavengeInventory()
        Configuration.State["Status"] = "[POTATO] Try " .. attempt .. " - P:" .. Potato .. " F:" .. Flour
        
        if Potato >= 1 and Flour >= 1 then
            Configuration.State["Status"] = "[POTATO] Ingredients obtained!"
            return true
        end
    ...
    Configuration.State["Status"] = "[ERROR] Failed to buy ingredients!"
    return false
end
...
local function StartPotatoJob()
    ...
    if ClipboardPrompt then
        pcall(function()
            ...
            ClipboardPrompt.HoldDuration = 0
        end)
    end

    Configuration.State["Status"] = "[POTATO] Going to Clipboard..."
    ...
    repeat
        ...
        task.wait(1)
        Attempts = Attempts + 1
        Configuration.State["Status"] = "[POTATO] Claiming task... " .. Attempts
        
        if Attempts > 20 then 
            Configuration.State["Status"] = "[ERROR] Clipboard failed!"
            break 
        end
...
-- ============================================
-- MAIN POTATO CONTROLLER (FIXED URUTAN)
-- ============================================
...
local function MainPotatoController()
    ...
    while Configuration.Main_Settings["Autofarming"] do
        ...
        if not IsOnBike() then
            Configuration.State["Status"] = "[WAIT] Naik bike dulu manual!"
            task.wait(2)
            continue
        end

        -- STEP 1: BELI BAHAN (WAJIB BERHASIL)
        local buySuccess = PurchasePotatoIngredients()
        if not buySuccess then
            Configuration.State["Status"] = "[RETRY] Gagal beli bahan..."
            task.wait(3)
            continue
        end
        
        -- STEP 2: VERIFIKASI BAHAN
        local Potato, Flour = ScavengeInventory()
        if Potato < 1 or Flour < 1 then
            Configuration.State["Status"] = "[RETRY] Bahan gak lengkap!"
            task.wait(3)
            continue
        end

        -- STEP 3: LANJUT PROSES
        StartPotatoJob()
        CutPotato()
        BagPotato()
        MixFlourAndPotato()
...
local function CreateUI()
    ...
    f.Size = UDim2.new(0, 320, 0, 400)
    f.Position = UDim2.new(0.5, -160, 0.5, -200)
    ...
    t.Text = "🍟 POTATO FARM (MANUAL BIKE)"
    ...
    Sec("INFO")
    local bikeStatus = Lab("🚲 Bike: OFF", Color3.fromRGB(255, 100, 100))
    local debugLabel = Lab("🔍 P:0 F:0", Color3.fromRGB(200, 200, 100))
    ...
    Tog("▶ Autofarming Potato", false, function(v)
        ...
        if v then
            if not IsOnBike() then
                Configuration.State["Status"] = "[WAIT] Naik bike dulu!"
                Configuration.Main_Settings["Autofarming"] = false
                return
            end
            task.spawn(function()
    ...
    h.InputChanged:Connect(function(i)
        if drag and (i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement) then
            ...
            f.Position = UDim2.new(0.5, -160 + ex, 0.5, -200 + ey)
        end
    end)
    ...
    task.spawn(function()
        ...
        while task.wait(1) do
            ...
            -- Update bike status
            if IsOnBike() then
                bikeStatus.Text = "🚲 Bike: ON"
                bikeStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
            else
                bikeStatus.Text = "🚲 Bike: OFF"
                bikeStatus.TextColor3 = Color3.fromRGB(255, 100, 100)
            end
            
            -- Update debug
            local P, F = ScavengeInventory()
            debugLabel.Text = "🔍 P:" .. P .. " F:" .. F
        end
    end)
end
...
print("========================================")
print("  ✅ POTATO FARM LOADED (FIXED)")
print("  ✅ MANUAL BIKE MODE")
print("  ✅ VEHICLE TP SYSTEM (FAST)")
print("  ✅ VERIFIKASI BAHAN + RETRY")
