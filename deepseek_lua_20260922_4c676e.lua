-- ============================================
-- WANZZ PROJECT - POTATO ONLY (NO SPAWN BIKE)
-- MANUAL SPAWN BIKE
...
local function VehicleTeleport(targetPosition)
    ...
    if not seat then 
        Configuration.State["Status"] = "[TP] Not on vehicle! Spawn bike dulu."
        return false 
    end
...
local function CreateUI()
    ...
    Sec("INFO")
    local info = Lab("⚠️ Naik bike dulu sebelum ON", Color3.fromRGB(255, 200, 100))
    local bikeStatus = Lab("🚲 Bike Status: OFF", Color3.fromRGB(255, 100, 100))
    ...
    h.InputChanged:Connect(function(i)
        if drag and (i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement) then
            ...
            f.Position = UDim2.new(0.5, -150 + ex, 0.5, -175 + ey)
        end
    end)
    ...
    task.spawn(function()
        ...
        while task.wait(1) do
            ...
            -- Update status bike
            if IsOnBike() then
                bikeStatus.Text = "🚲 Bike Status: ON"
                bikeStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
            else
                bikeStatus.Text = "🚲 Bike Status: OFF"
...
if getgenv().AutoRejoinerEnabled then
    ...
    task.spawn(function()
        ...
        task.wait(2)
        -- Langsung start controller tanpa spawn bike
        if IsOnBike() then
            MainPotatoController()
        else
            Configuration.State["Status"] = "[WAIT] Naik bike dulu!"
...
print("========================================")
print("  ✅ POTATO FARM LOADED")
print("  ✅ NO SPAWN BIKE - MANUAL MODE")
